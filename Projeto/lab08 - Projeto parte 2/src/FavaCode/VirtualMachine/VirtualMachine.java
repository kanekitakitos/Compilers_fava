    ...
    private void decode(byte[] bytecodes) {
        ...
        try {
            ...
            for (int i = 0; i < poolSize; i++) {
                ...
                // Variante 1: É do Tipo Double (8 bytes)
                if (typeTag == 1) {
                    double val = din.readDouble();
                    this.constantPool.add(new VMValue(val));
                }

                // Variante 3: É do Tipo String (Lê Length como 4 Bytes seguido de `Length` * 2 Bytes dos caracteres Unicode)
                else if (typeTag == 3) {

                    int strLen = din.readInt(); // Lê o número base de char(s)
                    StringBuilder sb = new StringBuilder();
                    for (int j = 0; j < strLen; j++)
                        sb.append(din.readChar()); // Append iterativo do buffer contíguo

                    this.constantPool.add(new VMValue(sb.toString()));

                } else
                    runtime_error("Tipo de constante desconhecido ou ilegal presente no bytecode corrompido: " + typeTag);
...
//*--------------- instructions / mnemónicas core ------------------------------------------------------------------------------------------
    ...
    private void exec_inst(Instruction inst) {
        if (trace) {
            System.out.println(String.format("%5s: %-15s Stack VM: %s", IP, inst, stack));
        }
